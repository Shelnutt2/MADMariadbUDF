...
my_bool median_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  if (args->arg_count != 1) {
    strcpy(message,
           "wrong number of arguments: median() requires one argument");
    return 1;
  }
  ...
  if (args->arg_type[0] != REAL_RESULT && args->arg_type[0] != INT_RESULT &&
    ...
    if (args->arg_type[0] == STRING_RESULT)
      strcpy(message, "median() requires a real or integer as parameter 1, "
                      "received STRING");
    else
      strcpy(message,
             "median() requires a decimal, real or integer as parameter 1");
    return 1;
  }
...
void median_reset(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                  char *is_error) {
  median_clear(initid, is_null, is_error);
  median_add(initid, args, is_null, is_error);
}

void median_add(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                char *is_error) {
  if (args->args[0] != NULL) {
    median_data *buffer = (median_data *)initid->ptr;
...
double median(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *is_error) {
  ...
  if (buffer->double_values != NULL && buffer->double_values->size() > 0) {
    ...
    std::cerr << "median() internal error, all vectors were null in computation"
              << std::endl;
    *is_error = 1;
  }
  return median;
...
/*long long median( UDF_INIT* initid, UDF_ARGS* args, char* is_null, char*
is_error )
{
  median_data* buffer = (median_data*)initid->ptr;
