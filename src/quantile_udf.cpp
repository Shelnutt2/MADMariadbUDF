...
void quantile_reset(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                    char *error);
...
/*long long quantile( UDF_INIT* initid, UDF_ARGS* args, char* is_null, char
 * *error );*/
}
...
my_bool quantile_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  if (args->arg_count != 2) {
    strcpy(message,
           "wrong number of arguments: quantile() requires two argument");
    return 1;
  }
  ...
  if (args->arg_type[0] != REAL_RESULT && args->arg_type[0] != INT_RESULT &&
    ...
    if (args->arg_type[0] == STRING_RESULT)
      strcpy(message, "quantile() requires a real, decimal, double or integer "
                      "as parameter 1, received STRING");
    else
      strcpy(message, "quantile() requires a real, decimal, double or integer "
                      "as parameter 1, received Decimal");
    return 1;
  }
  ...
  if (args->arg_type[1] != REAL_RESULT && args->arg_type[1] != INT_RESULT &&
    ...
    if (args->arg_type[1] == STRING_RESULT)
      strcpy(message, "quantile() requires a real, decimal, double or integer "
                      "as parameter 2, received STRING");
    else
      strcpy(message, "quantile() requires a real, decimal, double or integer "
                      "as parameter 2, received unknown");
    return 1;
  }
...
void quantile_add(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                  char *is_error) {
  if (args->args[0] != NULL) {
    quantile_data *buffer = (quantile_data *)initid->ptr;
...
double quantile(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                char *is_error) {
  quantile_data *buffer = (quantile_data *)initid->ptr;

  if (buffer->double_values != NULL && buffer->double_values->size() > 0) {
    std::vector<double> quantile =
        Quantile(buffer->double_values, {buffer->quantile});
    return quantile[0];
  } else if (buffer->int_values != NULL && buffer->int_values->size() > 0) {
    std::vector<long long> quantile =
        Quantile(buffer->int_values, {buffer->quantile});
    return quantile[0];
  }
  std::cerr << "quantile() internal error, all vectors were null in computation"
            << std::endl;
  *is_error = 1;
  return 0;
}
